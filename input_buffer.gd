...
# SCRIPT TAKEN FROM
# https://github.com/johnnyneverwalked/godot-input-buffer/tree/godot-4
# - micwu
...
func is_action_press_buffered(action: String, window=BUFFER_WINDOW) -> bool:
	...
	for event in InputMap.action_get_events(action):
		if event is InputEventKey:
			...
			if keyboard_timestamps.has(scancode):
				if Time.get_ticks_msec() - keyboard_timestamps[scancode] <= window:
